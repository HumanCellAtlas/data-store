...
class TestSecureSwagger(unittest.TestCase):
    def setUp(self):
        self.secure = os.path.join(pkg_root, 'auth.secure_all.json')
        self.hca_default = os.path.join(pkg_root, 'auth.hca_default.json')
    ...
    def test_auth_can_be_determined_from_swagger(self):
        ...
        secure_config = self.set_and_return_current_config(self.secure)
        with open(self.secure) as f:
            expected_secure_config = json.loads(f.read())
        assert secure_config == expected_secure_config

        # do the same for the hca defaults
        hca_config = self.set_and_return_current_config(self.hca_default)
        with open(self.hca_default) as f:
    ...
    def test_generate_swagger_consistency(self):
        ...
        """
        # change swagger to having all auth secure
        self.set_and_return_current_config(self.secure)
        with open(os.path.join(pkg_root, 'dss-api.yml'), 'r') as f:
            secure_swagger_contents = f.readlines()

        # change swagger to the hca defaults
        self.set_and_return_current_config(self.hca_default)
        with open(os.path.join(pkg_root, 'dss-api.yml'), 'r') as f:
            hca_default_swagger_contents = f.readlines()

        # change back to having all auth secure and make sure it's the same file
        self.set_and_return_current_config(self.secure)
        with open(os.path.join(pkg_root, 'dss-api.yml'), 'r') as f:
