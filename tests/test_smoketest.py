...
class Smoketest(unittest.TestCase):
    ...
    def test_smoketest(self):
        ...
        run(f"{self.venv_bin}hca dss upload "
            "--replica aws "
            "--staging-bucket $DSS_S3_BUCKET_TEST "
            f"--src-dir {bundle_dir} > upload.json")
        run(f"{self.venv_bin}hca dss download --replica aws --bundle-uuid $(jq -r .bundle_uuid upload.json)")
        for i in range(10):
            try:
        ...
        run(f"{self.venv_bin}hca dss download --replica gcp --bundle-uuid $(jq -r .bundle_uuid upload.json)")
        ...
        for replica in "aws", "gcp":
            ...
            res = run(f"{self.venv_bin}hca dss put-subscription "
                      "--callback-url https://example.com/ "
                      "--es-query '{}' "
                      f"--replica {replica}",
                      runner=check_output)
            sub_id = json.loads(res.decode())["uuid"]
            run(f"{self.venv_bin}hca dss get-subscriptions --replica {replica}")
            run(f"{self.venv_bin}hca dss delete-subscription --replica {replica} --uuid {sub_id}")
