...
aws s3 cp "${bundle_file}" s3://${DSS_S3_BUCKET_TEST}/"${BUNDLE_KEY}"
BUNDLE_FILE_ETAG=$(aws s3api head-object --bucket ${DSS_S3_BUCKET_TEST} --key "${BUNDLE_KEY}" | jq -r '.ETag | fromjson')
...
DSS_S3_BUCKET_REGION=$(aws s3api get-bucket-location --bucket ${DSS_S3_BUCKET_TEST} | jq -r 'if (.LocationConstraint == null) then "us-east-1" else .LocationConstraint end')
envsubst_vars='$BUNDLE_KEY $BUNDLE_FILE_ETAG $BUNDLE_FILE_SIZE $DSS_S3_BUCKET_TEST $DSS_S3_BUCKET_REGION'
