    ...
    def process_key(self, _key):
        ...
        if cache_flow.should_cache_file(file_metadata['content-type'], file_metadata['size']):
            ...
            if not checked_out:
                ...
                file_checkout(replica=self.replica, token=None, file_metadata=file_metadata, blob_path=blob_key)
                assert self._verify_blob_existance(self.handle, self.checkout_bucket, blob_key)
