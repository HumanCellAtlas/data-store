...
from .helpers import get_dst_bundle_prefix, get_src_object_name, get_copy_mode, CopyMode, copyInline, sanity_check_dst, \
    sanity_check_src, get_destination_bucket, parallel_copy
...
chainedawslambda.aws.add_client("parallel-copy-supervisor-dev", chainedawslambda.s3copyclient.S3ParallelCopySupervisorTask)
...
def post(uuid: str, json_request_body: dict, replica: str, version: str = None):
    ...
    if (dst_bucket == None):
        ...
        sanity_check_dst(handle, json_request_body)
    ...
    for file in files:
        ...
        if copy_mode == CopyMode.NO_COPY:
            log.debug("File already exists - no copy is necessary")
            continue
        elif copy_mode == CopyMode.COPY_ASYNC:
            log.debug("Copying a large file using multi-part copy")
