...
def post(json_request_body: dict,
    ...
    try:
        ...
        if ex.status_code == requests.codes.bad_request:
            logger.debug({'message': "Invalid Query Recieved."}, exc_info=ex, stack_info=True)
            ...
            logger.debug({'message': "Search Context Error."}, exc_info=ex, stack_info=True)
            ...
            logger.error({'message': "Elasticsearch Invalid Endpoint."}, exc_info=ex, stack_info=True)
            raise DSSException(requests.codes.service_unavailable,
                               "service_unavailable",
                               "Elasticsearch reached an invalid endpoint. Try again later.")
        else:
            logger.error({'message': "Elasticsearch Internal Server Error."}, exc_info=ex, stack_info=True)
