...
def ensure_hca_defaults_are_still_set():
    """To be run on travis to make sure that no one makes a PR with a custom swagger accidentally."""
    with open(os.path.join(pkg_root, 'auth.hca_default.json')) as f:
        endpoints_from_config_file = json.loads(f.read())
    if determine_auth_configuration_from_swagger() != endpoints_from_config_file:
        raise TypeError('Swagger file auth does not match HCA defaults.  Please modify accordingly.')
...
def main(argv=sys.argv[1:]):
    ...
    parser.add_argument('-t', '--travis', dest="travis", action='store_true', default=False,
                        help='Run this option on travis to check that the swagger file has the hca default auth.')
    o = parser.parse_args(argv)

    if o.travis:
        ensure_hca_defaults_are_still_set()
    else:
